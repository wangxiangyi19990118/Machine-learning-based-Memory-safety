...
path_file=glob.glob('f:/pictures/exe-R/*/*.exe') #获取当前文件夹下个数
...
for i in range(path_number):
    ...
    graphname = 'f:/pictures/T/'+str(i)+'.png'
    '''
    pe = pefile.PE('F:/pictures/exe-R/1T/executable.228.exe')
...
#    print(length,filename)
    a.fromfile(f, 25*64)  # 将文件读入数组a中，舍去余出的字节
    f.close()
    g = numpy.reshape(a, (int(len(a) / width), width))  # 将数组转为二维
    #b=numpy.array(buffer)
   # b=array.array("B")
   # g=numpy.reshape(b,(int(len(buffer)/128)),128)
    g = numpy.uint8(g)
